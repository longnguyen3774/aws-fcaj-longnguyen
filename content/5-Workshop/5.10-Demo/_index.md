---
title : "Video Demo"
date : 2026-07-12
weight : 10
chapter : false
pre : " <b> 5.10. </b> "
---

### Project Demo

Here is the live demo information of the fashion demand forecasting web application deployed on AWS Cloud.

#### Live Demo Website:
* **URL:** [d4032equ6gf0o.cloudfront.net](https://d4032equ6gf0o.cloudfront.net)
* **Account:** `director`
* **Password:** `password123`

---

### Project Demo Video

You can watch or download the demo video using the link below:

* **Google Drive Video Link:** [Video DEMO.mp4](https://drive.google.com/file/d/12hmmlfS411ktxYFR1H6hzErBIxVyU7e3/view?usp=sharing)

#### Embedded Video Player:

{{% notice info %}}
If the embedded player below does not load or asks for permission, please access it directly using the [Google Drive Link](https://drive.google.com/file/d/12hmmlfS411ktxYFR1H6hzErBIxVyU7e3/view?usp=sharing).
{{% /notice %}}

<iframe src="https://drive.google.com/file/d/12hmmlfS411ktxYFR1H6hzErBIxVyU7e3/preview" width="100%" height="480" allow="autoplay"></iframe>