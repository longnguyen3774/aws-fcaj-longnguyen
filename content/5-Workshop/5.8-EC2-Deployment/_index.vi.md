...
#### BƯỚC 3: Upload Mã nguồn và Cấu hình Biến môi trường (.env)

##### 3.1 Nén và Upload mã nguồn từ Máy cá nhân lên EC2:

Thực hiện các thao tác sau tại **máy cá nhân của bạn** (Terminal trên macOS/Linux, Git Bash hoặc PowerShell trên Windows):

```bash
# 1. Nén thư mục mã nguồn dự án (loại trừ node_modules, file cấu hình cục bộ .env và lịch sử git)
# Trên hệ điều hành macOS/Linux:
tar --exclude='node_modules' --exclude='.env' --exclude='.git' -czf app.tar.gz .

# Trên hệ điều hành Windows (Sử dụng PowerShell tích hợp lệnh Tar):
tar --exclude='node_modules' --exclude='.env' --exclude='.git' -czf app.tar.gz .

# 2. Upload file nén lên thư mục home của Ubuntu trên EC2 (thay đổi <PUBLIC_IP_EC2> tương ứng)
scp -i "fashion-key.pem" app.tar.gz ubuntu@<PUBLIC_IP_EC2>:/home/ubuntu/

```

Sau đó, quay lại cửa sổ **SSH Terminal của EC2** để tiến hành giải nén ứng dụng:

```bash
# 3. Tạo thư mục chứa ứng dụng
mkdir -p /home/ubuntu/app

# 4. Di chuyển file nén vào thư mục app và tiến hành giải nén
mv /home/ubuntu/app.tar.gz /home/ubuntu/app/
cd /home/ubuntu/app
tar -xzf app.tar.gz

# 5. Xóa file nén sau khi giải nén thành công
rm app.tar.gz
...
Nhập cấu hình kết nối thực tế của hệ thống. **Lưu ý thay thế toàn bộ thông tin trong dấu `<...>` bằng các giá trị thật cấu hình dịch vụ của bạn**:
...
> [!CAUTION]
> Tuyệt đối không commit tệp cấu hình `.env` hoặc chia sẻ công khai các mật khẩu và mã khóa để bảo vệ an toàn thông tin hạ tầng đám mây của bạn.
...
Sau khi cấu hình hoàn tất, thực hiện chạy các dòng lệnh sau để kiểm thử kết nối đầu cuối giữa máy chủ Web App EC2 và các tài nguyên đám mây khác. Hãy thay thế các giá trị bảo mật bằng thông tin thực tế của bạn:
...
*Kết quả mong đợi: Liệt kê đầy đủ các tệp pickle mô hình học máy.*
